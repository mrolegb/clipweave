...
class OptionsPanel(QGroupBox):
    ...
    def _populate(self) -> None:
        ...
        self.option_cells["smart_scene_threshold"] = add_option(self.option_grid, 9, "Smart scene threshold", self.smart_scene_threshold_spin, "Lower adjacent-frame similarity starts a new segment.")
        self.option_cells["crf"] = add_option(self.option_grid, 10, "CRF", self.crf_spin, "Encoding quality. Lower is larger and cleaner.")
        self.option_cells["preset"] = add_option(self.option_grid, 11, "Preset", self.preset_combo, "Encoding speed preset. Slow favors compression quality.")
        self.option_cells["keep_audio"] = add_option(self.option_grid, 12, "Keep audio", self.keep_audio_check, "Use source audio instead of rendering a silent montage.")
        self.option_cells["fade_transition"] = add_option(self.option_grid, 13, "Fade transitions", self.fade_transition_check, "Blend clips visually. Disabled automatically when audio is kept.")
        self.option_cells["smart_editing"] = add_option(self.option_grid, 14, "Smart editing", self.smart_editing_check, "Trim or skip clips that mostly repeat earlier sampled frames.")
        self.option_cells["smart_dedupe_segments"] = add_option(self.option_grid, 15, "Dedupe smart segments", self.smart_dedupe_segments_check, "Run a second visual duplicate pass after trimming.")
        self.option_cells["smart_reorder_segments"] = add_option(self.option_grid, 16, "Reorder smart segments", self.smart_reorder_segments_check, "Sort salvaged segments visually after smart editing.")
        self.option_cells["save_manifest"] = add_option(self.option_grid, 17, "Manifest", self.save_manifest_check, "Write a JSON manifest next to the output video.")
        self.option_cells["save_contact_sheet"] = add_option(self.option_grid, 18, "Contact sheet", self.save_contact_sheet_check, "Write one debug image with selected source frames.")
        self.option_cells["keep_work"] = add_option(self.option_grid, 19, "Temporary files", self.keep_work_check, "Keep intermediate render files for debugging.")
