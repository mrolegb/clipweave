...
class OptionsPanel(QGroupBox):
    ...
    def __init__(self) -> None:
        ...
        self.clothing_priority_combo = combo(["none", "less", "more"])
    ...
    def _populate(self) -> None:
        ...
        self.option_cells["clothing_priority"] = add_option(self.option_grid, 4, "Clothing priority", self.clothing_priority_combo, "Optionally prioritize lower or higher estimated clothing coverage.")
        self.option_cells["image_duration"] = add_option(self.option_grid, 5, "Image duration, sec", self.image_duration_spin, "How long each still image stays on screen.")
        self.option_cells["max_duration"] = add_option(self.option_grid, 6, "Max duration, sec", self.max_duration_spin, "Upper limit for selected media. 0 means no limit.")
        self.option_cells["target_threshold"] = add_option(self.option_grid, 7, "Target threshold", self.target_threshold_spin, "Higher values keep only media closer to the target.")
        self.option_cells["duplicate_threshold"] = add_option(self.option_grid, 8, "Duplicate threshold", self.duplicate_threshold_spin, "Higher values remove only very close visual duplicates.")
        self.option_cells["smart_max_known_ratio"] = add_option(self.option_grid, 9, "Smart max known ratio", self.smart_max_known_ratio_spin, "Drop clips above this already-seen frame ratio.")
        self.option_cells["smart_min_segment_duration"] = add_option(self.option_grid, 10, "Smart min segment, sec", self.smart_min_segment_duration_spin, "Shortest repeated-clip segment worth keeping.")
        self.option_cells["smart_scene_threshold"] = add_option(self.option_grid, 11, "Smart scene threshold", self.smart_scene_threshold_spin, "Lower adjacent-frame similarity starts a new segment.")
        self.option_cells["crf"] = add_option(self.option_grid, 12, "CRF", self.crf_spin, "Encoding quality. Lower is larger and cleaner.")
        self.option_cells["preset"] = add_option(self.option_grid, 13, "Preset", self.preset_combo, "Encoding speed preset. Slow favors compression quality.")
        self.option_cells["keep_audio"] = add_option(self.option_grid, 14, "Keep audio", self.keep_audio_check, "Use source audio instead of rendering a silent montage.")
        self.option_cells["fade_transition"] = add_option(self.option_grid, 15, "Fade transitions", self.fade_transition_check, "Blend clips visually. Disabled automatically when audio is kept.")
