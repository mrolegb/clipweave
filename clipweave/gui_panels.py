...
class OptionsPanel(QGroupBox):
    ...
    def __init__(self) -> None:
        ...
        self.structure_combo = combo(["smooth", "arc", "variety"])
        ...
        self.smart_scene_threshold_spin = double_spin(-1.0, 1.0, 0.55, 2)
        ...
        self.split_aspects_check = QCheckBox("Split aspect ratios")
        self.exclude_manifest_history_check = QCheckBox("Exclude manifest history")
        self.auto_grade_check = QCheckBox("Auto grade")
        ...
        self.save_manifest_check = QCheckBox("Save manifest")
        self.save_contact_sheet_check = QCheckBox("Save contact sheet")
        ...
        style_checkbox(self.split_aspects_check)
        style_checkbox(self.exclude_manifest_history_check)
        style_checkbox(self.auto_grade_check)
        ...
        style_checkbox(self.save_manifest_check)
        style_checkbox(self.save_contact_sheet_check)
    ...
    def apply_media_rules(self, media: str) -> None:
        ...
        self._set_option_enabled("smart_scene_threshold", self.smart_editing_check.isChecked())
    ...
    def apply_smart_rules(self, enabled: bool) -> None:
        ...
        self._set_option_enabled("smart_scene_threshold", enabled)
    ...
    def _populate(self) -> None:
        ...
        self.option_cells["structure"] = add_option(self.option_grid, 3, "Structure", self.structure_combo, "Choose smooth continuity, a motion arc, or more variety.")
        self.option_cells["image_duration"] = add_option(self.option_grid, 4, "Image duration, sec", self.image_duration_spin, "How long each still image stays on screen.")
        self.option_cells["max_duration"] = add_option(self.option_grid, 5, "Max duration, sec", self.max_duration_spin, "Upper limit for selected media. 0 means no limit.")
        self.option_cells["target_threshold"] = add_option(self.option_grid, 6, "Target threshold", self.target_threshold_spin, "Higher values keep only media closer to the target.")
        self.option_cells["duplicate_threshold"] = add_option(self.option_grid, 7, "Duplicate threshold", self.duplicate_threshold_spin, "Higher values remove only very close visual duplicates.")
        self.option_cells["smart_max_known_ratio"] = add_option(self.option_grid, 8, "Smart max known ratio", self.smart_max_known_ratio_spin, "Drop clips above this already-seen frame ratio.")
        self.option_cells["smart_min_segment_duration"] = add_option(self.option_grid, 9, "Smart min segment, sec", self.smart_min_segment_duration_spin, "Shortest repeated-clip segment worth keeping.")
        self.option_cells["smart_scene_threshold"] = add_option(self.option_grid, 10, "Smart scene threshold", self.smart_scene_threshold_spin, "Lower adjacent-frame similarity starts a new segment.")
        self.option_cells["crf"] = add_option(self.option_grid, 11, "CRF", self.crf_spin, "Encoding quality. Lower is larger and cleaner.")
        self.option_cells["preset"] = add_option(self.option_grid, 12, "Preset", self.preset_combo, "Encoding speed preset. Slow favors compression quality.")
        self.option_cells["keep_audio"] = add_option(self.option_grid, 13, "Keep audio", self.keep_audio_check, "Use source audio instead of rendering a silent montage.")
        self.option_cells["fade_transition"] = add_option(self.option_grid, 14, "Fade transitions", self.fade_transition_check, "Blend clips visually. Disabled automatically when audio is kept.")
        self.option_cells["split_aspects"] = add_option(self.option_grid, 15, "Split aspects", self.split_aspects_check, "Build one output per source aspect-ratio group.")
        self.option_cells["exclude_manifest_history"] = add_option(self.option_grid, 16, "Manifest history", self.exclude_manifest_history_check, "Skip source files already listed in other manifests in the input folder.")
        self.option_cells["auto_grade"] = add_option(self.option_grid, 17, "Auto grade", self.auto_grade_check, "Lightly normalize brightness, contrast, and saturation during render.")
        self.option_cells["smart_editing"] = add_option(self.option_grid, 18, "Smart editing", self.smart_editing_check, "Trim or skip clips that mostly repeat earlier sampled frames.")
        self.option_cells["smart_dedupe_segments"] = add_option(self.option_grid, 19, "Dedupe smart segments", self.smart_dedupe_segments_check, "Run a second visual duplicate pass after trimming.")
        self.option_cells["smart_reorder_segments"] = add_option(self.option_grid, 20, "Reorder smart segments", self.smart_reorder_segments_check, "Sort salvaged segments visually after smart editing.")
        self.option_cells["save_manifest"] = add_option(self.option_grid, 21, "Manifest", self.save_manifest_check, "Write a JSON manifest next to the output video.")
        self.option_cells["save_contact_sheet"] = add_option(self.option_grid, 22, "Contact sheet", self.save_contact_sheet_check, "Write one debug image with selected source frames.")
        self.option_cells["keep_work"] = add_option(self.option_grid, 23, "Temporary files", self.keep_work_check, "Keep intermediate render files for debugging.")
